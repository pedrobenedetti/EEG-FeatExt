...
SUBJECTS_TO_RUN = ["01_test_2023"]  # None = todos los sujetos de config_sujetos.py
...
STATUS_MASK = 0xFFFF  # Bits de eventos BioSemi; None si no corresponde a otro equipo.
MARK_TOLERANCE_SECONDS = 0.1  # Verificar con los tiempos reales; no desplaza ventanas.
WINDOW_SECONDS = 5.0
REMAINDER_POLICY = "error"  # "error" o "drop" (descarta solo resto final, lo registra).
PROTOCOL = {
...
WPLI_MIN_EPOCHS = 2  # Control matematico, NO garantia de fiabilidad estadistica.
EXPORT_CSV = False
REUSE_CLEAN_CACHE = True
PREPROCESS = dict(excluded=["EXG3", "EXG4", "EXG5", "EXG6", "EXG7", "EXG8"],
                  lowpass_cut=1, highpass_cut=30, interpolate=True)
ICA_SETTINGS = dict(method="infomax", n_components=15, decim=3, random_state=23,
                    reject_limit=250e-6)
# Los nombres lowpass_cut/highpass_cut son historicos: significan 1 Hz y 30 Hz.
...
LZC_SETTINGS = dict(freq_range=(1, 30), min_samples=200)  # LZC de banda ancha original.
TE_SETTINGS = dict(maxlag_ms=400.0, maxlag=None, min_obs_needed=30, biascorrect=True)
